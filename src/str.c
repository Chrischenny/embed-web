...
struct emb_str emb_str_s(const char *s)
{
    struct emb_str str = {s, s == NULL ? 0 : strlen(s)};
    return str;
}

struct emb_str emb_str_n(const char *s, size_t n)
{
    struct emb_str str = {s, n};
    return str;
}
...
int emb_ncasecmp(const char *s1, const char *s2, size_t len)
{
    ...
    if (len > 0)
        do
        {
            diff = emb_lower(s1++) - emb_lower(s2++);
...
{
    return emb_ncasecmp(s1, s2, (size_t)~0);
}
...
int emb_vcasecmp(const struct emb_str *str1, const char *str2)
{
    ...
    int r = emb_ncasecmp(str1->ptr, str2, (n1 < n2) ? n1 : n2);
    if (r == 0)
        return (int)(n1 - n2);
    return r;
}
...
const char *emb_strstr(const struct emb_str haystack,
                       const struct emb_str needle)
{
    size_t i;
...
bool emb_match(struct emb_str s, struct emb_str p, struct emb_str *caps)
{
...
bool emb_globmatch(const char *s1, size_t n1, const char *s2, size_t n2)
{
    return emb_match(emb_str_n(s2, n2), emb_str_n(s1, n1), NULL);
}

static size_t emb_nce(const char *s, size_t n, size_t ofs, size_t *koff,
...
bool emb_split(struct emb_str *s, struct emb_str *k, struct emb_str *v, char sep)
{
    size_t koff = 0, klen = 0, voff = 0, vlen = 0, off = 0;
    if (s->ptr == NULL || s->len == 0)
        return 0;
    off = emb_nce(s->ptr, s->len, 0, &koff, &klen, &voff, &vlen, sep);
    if (k != NULL)
        *k = emb_str_n(s->ptr + koff, klen);
    if (v != NULL)
        *v = emb_str_n(s->ptr + voff, vlen);
    *s = emb_str_n(s->ptr + off, s->len - off);
    return off > 0;
}

bool emb_commalist(struct emb_str *s, struct emb_str *k, struct emb_str *v)
{
    return emb_split(s, k, v, ',');
}

size_t emb_snprintf(char *buf, size_t len, const char *fmt, ...)
{
    va_list ap;
    size_t n;
    va_start(ap, fmt);
    n = emb_vsnprintf(buf, len, fmt, &ap);
    va_end(ap);
    return n;
}

char *emb_hex(const void *buf, size_t len, char *to)
{
...
static unsigned char emb_unhex_nimble(unsigned char c)
{
...
unsigned long emb_unhexn(const char *s, size_t len)
{
    ...
    for (i = 0; i < len; i++)
        v <<= 4, v |= emb_unhex_nimble(((uint8_t *)s)[i]);
    return v;
}
...
void emb_unhex(const char *buf, size_t len, unsigned char *to)
{
    ...
    for (i = 0; i < len; i += 2)
    {
        to[i >> 1] = (unsigned char)emb_unhexn(&buf[i], 2);
    }
}
...
size_t emb_vasprintf(char **buf, size_t size, const char *fmt, va_list ap)
{
    ...
    va_copy(ap_copy, ap);
    len = emb_vsnprintf(*buf, size, fmt, &ap_copy);
    va_end(ap_copy);
    ...
    if (len >= size)
    {
        ...
        {
            va_copy(ap_copy, ap);
            len = emb_vsnprintf(*buf, len + 1, fmt, &ap_copy);
            va_end(ap_copy);
        }
    }
...
size_t emb_asprintf(char **buf, size_t size, const char *fmt, ...)
{
    size_t ret;
    va_list ap;
    va_start(ap, fmt);
    ret = emb_vasprintf(buf, size, fmt, ap);
    va_end(ap);
    return ret;
}
...
static size_t emb_copys(char *buf, size_t len, size_t n, char *p, size_t k)
{
    size_t j = 0;
    for (j = 0; j < k && p[j]; j++)
        if (j + n < len)
...
static char emb_esc(int c, bool esc)
{
...
static char emb_escape(int c)
{
    return emb_esc(c, true);
}

static size_t emb_copyq(char *buf, size_t len, size_t n, char *p, size_t k)
{
    size_t j = 0, extra = 2;
    if (n < len)
        buf[n++] = '"';
...
typedef size_t (*emb_spfn_t)(char *, size_t, va_list *);
        ...
        if (fmt[i] == '%')
        {
                ...
                c == 'g')
            {
                ...
                if (c == 'g' || c == 'f')
                {
                    ...
                    if (pr == ~0U)
                        pr = 6;
                    k = emb_dtoa(tmp, sizeof(tmp), v, (int)pr);
                }
                else if (is_long == 2)
                {
                ...
                else if (is_long == 1)
                {
                    ...
                    k = emb_lld(tmp, s ? (int64_t)v : (int64_t)(unsigned long)v, s, h);
                }
                else
                {
                    int v = va_arg(*ap, int);
                    k = emb_lld(tmp, s ? (int64_t)v : (int64_t)(unsigned)v, s, h);
                }
                for (j = 0; j < xl && w > 0; j++)
                    w--;
                for (j = 0; pad == ' ' && !minus && k < w && j + k < w; j++)
                    n += emb_copys(buf, len, n, &pad, 1);
                n += emb_copys(buf, len, n, (char *)"0x", xl);
                for (j = 0; pad == '0' && k < w && j + k < w; j++)
                    n += emb_copys(buf, len, n, &pad, 1);
                n += emb_copys(buf, len, n, tmp, k);
                for (j = 0; pad == ' ' && minus && k < w && j + k < w; j++)
                    n += emb_copys(buf, len, n, &pad, 1);
            }
            else if (c == 'M')
            {
                ...
                emb_spfn_t fn = va_arg(*ap, emb_spfn_t);
                n += fn(dst, dstlen, ap);
            }
            else if (c == 'c')
            {
...
double emb_atod(const char *p, int len, int *numlen)
{
    double d = 0.0;
    int i = 0, sign = 1;
...
size_t emb_dtoa(char *dst, size_t dstlen, double d, int width)
{
    char buf[40];
    int i, s = 0, n = 0, e = 0;
    double t, mul, saved;
    if (d == 0.0)
        return emb_snprintf(dst, dstlen, "%s", "0");
    if (xisinf(d))
        return emb_snprintf(dst, dstlen, "%s", d > 0 ? "inf" : "-inf");
    if (xisnan(d))
        return emb_snprintf(dst, dstlen, "%s", "nan");
    if (d < 0.0)
        d = -d, buf[s++] = '-';
    ...
    if (e >= width)
    {
        n = (int)emb_dtoa(buf, sizeof(buf), saved / mul, width);
        // printf(" --> %.*g %d [%.*s]\n", 10, d / t, e, n, buf);
        n += addexp(buf + s + n, e, '+');
        return emb_snprintf(dst, dstlen, "%.*s", n, buf);
    }
    else if (e <= -width)
    {
        n = (int)emb_dtoa(buf, sizeof(buf), saved / mul, width);
    ...
    if (n > 0 && buf[s + n - 1] == '.')
        n--; // Trim trailing dot
    buf[s + n] = '\0';
    return emb_snprintf(dst, dstlen, "%s", buf);
}
